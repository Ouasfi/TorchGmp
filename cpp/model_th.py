...
def einsum_ij(a, b):
    ...
    n_a = a.size(1)
    n_b = b.size(1)
    return torch.mul(a.repeat(n_b, 1,1).permute(1,2,0).flatten(),b.repeat(n_a,1,1).permute(1,0,2).flatten()).reshape(batch, n_a,n_b)
class IntegerMLP(object):
    def __init__(self, layers=[2, 10, 1], activations=['sigmoid', 'sigmoid'], t = 5):
        ...
        for i in range(len(layers) - 1):
            self.weights.append(
                (torch.randn( size=(layers[i + 1], layers[i]))*q_factor*np.sqrt(2 / layers[i])
                ).long())
            self.biases.append(
                (torch.randn( size=(layers[i + 1], 1))*q_factor* np.sqrt(2 / layers[i])).long())
    ...
    def feedforward(self, x):
        ...
        for i in range(len(self.weights)):
            ...
            #print(torch.matmul(a, self.weights[i].T).shape, self.biases[i].shape)
            z_s.append(a.matmul( self.weights[i].t())// q_factor + self.biases[i].long().squeeze(-1))
    ...
    def backprop(self, input, target):
        ...
        for i in reversed(range(len(deltas) - 1)):
            deltas[i] =  deltas[i + 1].matmul( self.weights[i + 1],) * (
                self.get_derivative_activation_function(self.activations[i])(z_s[i]))// q_factor
        db = deepcopy(deltas)  # dC/dW
        
        #torch.einsum('bi,bj->bij',  deltas[i], a_s[i])// q_factor
        dw = [ einsum_ij(deltas[i],a_s[i] )// q_factor for i, d in enumerate(deltas)]  # dC/dB
...
if __name__ == "__main__":
    ...
    train_data = (torch.tensor(train_data).squeeze(-1).long())
    train_labels = (torch.tensor(train_labels).squeeze(-1).long())
