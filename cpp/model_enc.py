...
if __name__ == "__main__":
    ...
    nn = EncryptedMLP([2, 10, 2], activations=['relu', 'linear'], t = 2)
    train_data = EncryptedTensor(torch.tensor(train_data).squeeze(-1).long(), q_factor)
    train_labels = EncryptedTensor(torch.tensor(train_labels).squeeze(-1).long(), q_factor)
